...
import { login, isLoggedIn, hasScopes, logout, getStoredTokens } from '../auth/pkce';
...
function ConnectScreen({ reconnect }: { reconnect: boolean }) {
  ...
  return (
    ...
    >
      ...
      <p>
        {reconnect
          ? 'This report needs permissions your current session doesn’t have yet.'
          : 'See how your Spotify taste is shifting.'}
      </p>
      <button
        type="button"
        className="primary"
      ...
      >
        {reconnect ? 'Reconnect Spotify' : 'Log in with Spotify'}
      </button>
...
function TopDataView() {
  ...
  return (
    <main style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
      <Link to="/" style={{ fontSize: 13, display: 'inline-block', marginBottom: 12 }}>
        ← Now playing
      </Link>
...
export default function ReportPage() {
  if (!isLoggedIn()) return <ConnectScreen reconnect={false} />;
  if (!hasScopes(REQUIRED_SCOPES)) return <ConnectScreen reconnect={true} />;
  return <TopDataView />;
}
